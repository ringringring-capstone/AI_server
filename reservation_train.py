...
class CharDataset(Dataset):
    def __init__(self, chats, max_len=75):
        self._data = chats
        self.first = True
        self.q_token = U_TKN
        self.a_token = S_TKN
...
class KoGPT2Chat(LightningModule):
    def __init__(self, hparams, **kwargs):
        ...
        self.neg = torch.tensor(-1e18, requires_grad=True)
        self.kogpt2 = GPT2LMHeadModel.from_pretrained('skt/kogpt2-base-v2')
        self.loss_function = torch.nn.CrossEntropyLoss(reduction='none')

        
        # Ensure all model parameters have requires_grad=True
        for param in self.kogpt2.parameters():
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--max-len',
                            type=int,
                            default=75,
                            help='max sentence length on input (default: 55)')
    ...
    def training_step(self, batch, batch_idx):
        ...
        out = self(token_ids).requires_grad_(True)      
        ...
        return loss_avg / self.trainer.accumulate_grad_batches
    ...
    def train_dataloader(self):
        data = pd.read_csv('data/reservation_final.csv')
...
if __name__ == "__main__":
    if args.train:
        ...
        trained_model.save_pretrained('model/checkpoint/')
    if args.chat:
